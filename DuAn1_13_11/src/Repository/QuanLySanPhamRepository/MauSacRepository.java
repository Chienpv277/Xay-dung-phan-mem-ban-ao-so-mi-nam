...
public class MauSacRepository {
    ...
    public Boolean add(MauSac mauSac) {
        String sql = "insert into mau_sac(ten) values (?)";
        String maMauSac = genMauSac();
        try ( Connection con = connection.getConnection();  PreparedStatement ps = con.prepareStatement(sql)) {
    ...
    public String genMauSac() {
        String mauSac = "mausac";
        String ma = "";
        String sql = "SELECT COALESCE(MAX(CAST(SUBSTRING(ma, LENGTH(?)+1) AS SIGNED)), 0)"
                + " FROM MauSac";
        try ( Connection con = connection.getConnection();  PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, mauSac);
            try ( ResultSet resultSet = ps.executeQuery()) {
                if (resultSet.next()) {
                    int numericPart = resultSet.getInt(1);
                    ma = mauSac + (numericPart + 1);
                }
            }
        } catch (Exception e) {
            e.getMessage();
